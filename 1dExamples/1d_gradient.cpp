...
int main(int argc, char* argv[])
{
    ...
    if (argc != 5)
    {
        std::cout << "input arguments: numElements, displacement order, nl eq strain order, integration order" << std::endl;
        return 1;
    }
...
//    myStructure.AddVisualizationComponentConstitutive();
    ...
    maxStressOutput << numElements << "\t" << dispOrder << "\t" << nlOrder << "\t" << ipOrder << "\t" << dofs << "\t" << maxStress << std::endl;
    ...
    sprintf(forceDispName, "forceDisp_ele_%03d_disp_%02d_nl_%02d_ip_%02d.dat", numElements, dispOrder, nlOrder,
            ipOrder);
    std::string command = "paste " + resultDirectory.string() + "myforce.dat " + resultDirectory.string() + "mydisplacements.dat > " + resultDirectory.string() + forceDispName;
    ...
    IntegralOutput <<   numElements << "\t\t" << dispOrder << "\t\t" << nlOrder << "\t\t" << ipOrder << "\t\t" << dofs << "\t\t" <<std::setprecision(10) << integral << "\t\t" << std::chrono::duration_cast<std::chrono::milliseconds>(end-begin).count() <<std::endl;
